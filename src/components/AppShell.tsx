...
import { LayoutDashboard, Radar, Droplets, TrendingUp, SlidersHorizontal, ArrowLeftRight, Building2, Workflow as WorkflowIcon, FileSearch, ScrollText, CircleDollarSign, RotateCcw } from "lucide-react";
...
function Sidebar({
  ...
  return <aside className="flex h-full w-[228px] shrink-0 flex-col bg-panel text-panel-foreground">
      <div className="flex items-center gap-2.5 border-b border-panel-border px-4 py-4">
        <div className="flex h-8 w-8 items-center justify-center rounded-[3px] border border-brand/60 text-[13px] font-bold text-brand-foreground" style={{
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        {NAV.map(item => <NavLink key={item.to} to={item.to} end={item.to === "/"} className={({
        isActive
      }) => cn("mb-[2px] flex items-center gap-2.5 rounded-[3px] px-2.5 py-[7px] text-[12.5px] font-medium transition-colors", isActive ? "bg-panel-foreground/10 text-panel-foreground shadow-[inset_2px_0_0_0_hsl(var(--brand))]" : "text-panel-foreground/55 hover:bg-panel-foreground/5 hover:text-panel-foreground")}>
      ...
      <div className="border-t border-panel-border px-4 py-3 text-[11px] leading-relaxed">
        ...
        <div className="mt-3 flex gap-1.5">
          <Button variant="ghost" size="sm" className="h-7 flex-1 border border-panel-border text-[11px] text-panel-foreground/60 hover:bg-panel-foreground/5 hover:text-panel-foreground" onClick={() => {
          reset.mutate();
...
function Header({
  ...
  return <header className="flex h-[52px] shrink-0 items-center justify-between border-b bg-card px-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden leading-tight md:block">
            <div className="text-[11.5px] font-semibold">{"Nischala GS\n"}</div>
      ...
      <div className="flex h-full w-full overflow-hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <main className="flex-1 overflow-y-auto bg-background">
            {isLoading ? <div className="p-6 text-[13px] text-muted-foreground">Loading financial state…</div> : isError ? <div className="p-6">
              ...
              </div> : <div className="mx-auto max-w-[1400px] px-6 py-6">
                <Outlet context={{
              state
            }} />
              </div>}
